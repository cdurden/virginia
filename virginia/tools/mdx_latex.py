    ...
    def extendMarkdown(self, md, md_globals):
        ...
        for (key, pat) in self.md.inlinePatterns.items():
            ...
            if pat.pattern == markdown.inlinepatterns.ESCAPE_RE:
                ...
                #idx = self.md.inlinePatterns.index(key)
        ...
        md.treeprocessors.add("latex",treeprocessor,"<prettify")
...
#        md.postprocessors.add("math",math_pp,"_end")
        md.postprocessors.add("table",table_pp,"_end")
        md.postprocessors.add("image",image_pp,"_end")
        #md.textPostprocessors.append(math_pp)
        #md.textPostprocessors.append(table_pp)
        #md.textPostprocessors.append(image_pp)
        # run last
        md.postprocessors.add("unescape_html",unescape_html_pp,"_end")
...
class LaTeXTreeProcessor(markdown.treeprocessors.Treeprocessor):
    ...
    def run(self, tree):
        print("running latex tree processor");
...
#            latex_text = self.tolatex(currElement)
...
#        latex_node = util.etree.Element('span')
#        latex_node.text = latex_text
#        doc.appendChild(latex_node)
#        stack.append(latex_node)
#        return(tree)
        return(self.tolatex(tree))
#    def run(self, doc):
#        '''Walk the dom converting relevant nodes to text nodes with relevant
#        content.'''
#        #latex_text = self.tolatex(doc.documentElement)
...
#        return(latex_text)
    ...
    def tolatex(self, tree):
        ...
        if tree.text:
            ...
            tree.text = escape_latex_entities(tree.text)
            return tree
...
#% ----------------------------------------------------------------
#'''
        hs = tree.getiterator('h1')
        subcontent = ""
        for h in hs:
            subcontent += self.tolatex(h).text
            h.text = '\n\n\\section{%s}\n' % subcontent

        ps = tree.getiterator('p')
        subcontent = ""
        for p in ps:
            subcontent += self.tolatex(p).text
            p.text = '\n\n\\section{%s}\n' % subcontent
...
#        index = md.preprocessors.index(md_globals['REFERENCE_PREPROCESSOR'])
        ...
        preprocessor.md = md
        md.preprocessors.add("footnote", preprocessor,"<reference")
...
#        md.inlinePatterns.insert(index, FootnotePattern(FOOTNOTE_RE, self))
        md.inlinePatterns.add("footnote", FootnotePattern(FOOTNOTE_RE, self),"<image_reference")
